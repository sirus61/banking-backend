...
const getUsers = async (req, res) => {
  try {
    const users = await User.find().select('_id user_name email user_status no_of_account')
...
const userLogin = async (req, res) => {
  ...
  if (!req.body.email || !req.body.password) return res.status(404).send('empty body request')
  ...
  try {
    ...
    if (!user || !isCorrectPassword) return res.status(404).send('Wrong Credintials - wrong email or password')
    res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  }
}
...
const updateUser = async (req, res) => {
  try {
    ...
    if (error.message.match(/(email|password|name|postal|phone|addresee)/gi)) return res.status(400).send(error.message)
    res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  }
}
...
const notificationUpdate = async (req, res) => {
  try {
    ...
    if (error.message.match(/(notification)/gi)) return res.status(400).send(error.message)
    res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  }
}

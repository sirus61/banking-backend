...
const adminLogin = async (req, res) => {
  ...
  try {
    ...
    if (!admin || !isCorrectPassword) {
      return res.status(404).send('Wrong Credintials - wrong email or password')
    }
...
//@route  >>>> GET /api/admins/getuserdetails/:id
//@Access >>>> private(Owner Only)
const getAllUserDetails = async (req, res) => {
  try {
    ...
    const accounts = await Account.find({ client_id: userId })

    // Fetch transactions related to this user's accounts

...
const transfer = async (req, res, next) => {
  ...
  try {
    ...
    if (error.message.match(/(transfer|id|Balance|Account)/gi)) return res.status(400).send(error.message)
    res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  }
}
...
const deposit = async (req, res) => {
  ...
  try {
    ...
    if (error.message.match(/(Balance|Account)/gi)) return res.status(400).send(error.message)
    res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  }
}

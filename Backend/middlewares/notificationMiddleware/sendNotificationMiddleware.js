...
const sendNotification = async (req, res) => {
  ...
  //     console.log(req.approved.client_id)
  //     //update user (no_of_account + accounts IDS array)
  //     user.no_of_account += 1;
  //     user.markModified("no_of_account");
  //     console.log("approved", req.approved.account_id)
  //     const usertwo = user.accounts.push(req.approved.account_id);
  ...
  if (req.approved) {
    try {
      console.log('Processing account approval...')
      const user = await User.findById(req.approved.client_id)
      console.log('User found for approval:', user)
      ...
      user.markModified('notifications')
      console.log('User after modifications (before save):', user)

      // const savedUser = await user.save();
      // console.log("User saved with new account and notification", savedUser);
      ...
      const deletedAccountRequest = await AccountRequest.findByIdAndDelete(req.approved.request_id)
      console.log('Deleted account request:', deletedAccountRequest)

      res.status(200).json({ id: deletedAccountRequest.id })
    } catch (error) {
      console.error('Error during account approval processing:', error)
      if (error.message.match(/(notification|Profile)/gi)) {
        return res.status(400).send(error.message)
      }
      res.status(500).send('Ooops!! Something Went Wrong, Try again...')
  ...
  if (req.transfered) {
    try {
      ...
      const ReceivingUser = await User.findById(req.transfered.updatedReceivingAccount.client_id)
